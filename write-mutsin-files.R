...
# chr.name.sub.wtsd <- "sub.wtsd.02"
# 
# ## load data.frame
# df.cur <- lst.loads["sub.wtsd.02"][[1]]
# 
# write.mutsin(df.cur=lst.loads["sub.wtsd.03"][[1]], 
#              chr.dir.hspf.mutsin = chr.dir.hspf, 
#              chr.file.uci.mutsin = chr.file.uci,
#              chr.name.MUSTIN = "direct03.mut", 
#              chr.name.wtsd = "Upper Yaquina River",
#              chr.name.sub.wtsd = "sub.wtsd.03")

write.mutsin <- function(df.cur, chr.dir.hspf.mutsin, chr.file.uci.mutsin,
                         chr.name.MUSTIN, chr.name.wtsd, chr.name.sub.wtsd) {
