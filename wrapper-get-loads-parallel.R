wrapper.get.loads.parallel <- function(chr.prefix, chr.pls.names, lst.output,
                                       chr.dir.source.control.scripts) {
  library(data.table)
  library(parallel)
  source(paste0(chr.dir.source.control.scripts, "/",
                "get-loads-parallel.R"))
  ...
  df.loads <- rbindlist(parLapply(c1, lst.output$sub.wtsd.names, get.loads.parallel,
