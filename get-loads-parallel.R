get.loads.parallel <- function(chr.sub.wtsd, chr.prefix, chr.pls.names, 
  ...
  if(length(
    ...
    try(stopCluster(c11), silent = TRUE)
    
    ## create cluster
    no_cores <- detectCores() - 1
    if(no_cores < 1) no_cores <- 1
    c11 <- makeCluster(no_cores)
    ...
    chr.cur.sources <- do.call(rbind,
                               parLapply(c11,1:length(lst.cur.wtsd), 
    ...
    lst.sub.loads <- parLapply(c11,chr.cur.sources, get.loads.source,
